...
server.py (Starlette+uvicorn) ← HTTP + WebSocket on configurable host:port (default localhost:8765; Docker/non-loopback supported via OUROBOROS_SERVER_HOST=0.0.0.0)
  ...
  └── ouroboros/               ← Agent core (runs inside worker processes)
      ...
      ├── _outcome_tool_errors.py ← Tool-call trace vocabulary and the execution-axis error classifier (extracted from outcomes.py at the v6.90.x submarine unwind so the typed-outcome module stays under the hard module gate): the status/tool frozensets that partition a tool call into blocking / policy-denial / cosmetic / ignored buckets plus the classifier that walks a trace and applies them; a LEAF that never imports `ouroboros.outcomes` — `outcomes` re-exports every name here so historical import sites (`agent_task_pipeline`, the tests) keep working unchanged
      ...
      ├── _usage_rows.py       ← Pure row-math projections over ledger rows (extracted from usage_accounting.py at the v6.91 gate-3 fix round to hold the module gate): the accounted/summary arithmetic (`_summary`), limit/integrity decorations (`_with_limit`/`_with_integrity`), and per-row physical-call/bucket projections (`_physical_call_count`/`_breakdown_bucket`); a LEAF over plain row dicts — no file I/O, no locks, never imports `usage_accounting`, which re-exports every name so historical import sites keep working unchanged
      ...
      ├── context_layout.py    ← Reference-doc layout SSOT (D-ARCH, 2026-08-08): ARCHITECTURE follows the owner mode alone (full in max for every task class — the capability/tools/access map; nav map in low); DEVELOPMENT full/pointer is the caller's mode-independent per-class decision, keyed on the ACTIVE REPO BINDING (D-DEV: pointer for the external-surface class — bound workspace / subagent / api-cli-scheduled — full whenever the work is still on Ouroboros's own body, including a direct-chat turn in a project room); README/CHECKLISTS on-demand pointers
      ├── context_compaction.py ← Context trimming and summarization helpers
      ├── context_health.py    ← Health-invariant probes and the log-analysis checks they read (extracted from context.py at the v6.90.x submarine unwind so the context builder stays under the hard module gate): a LEAF that reads runtime logs, version carriers and custody ledgers and renders the "## Health Invariants" section; never imports `ouroboros.context` — `context` re-exports `safe_read`/`build_health_invariants` so every historical import site (`consciousness`, tests monkeypatching `context._STRAY_PROBE_CACHE`) keeps working unchanged
      ├── headless.py          ← Headless task child-drive isolation, workspace patch artifacts, and memory export helpers
      ├── coop_checkpoint.py   ← (v6.58.0) `checkpoint_commit_coop_roots` — dirty host-minted genesis/coop trees get a local checkpoint commit (`user.name=Ouroboros`); credential-shaped files excluded (disclosed, headless sensitive-pattern SSOT), owner-attached folders NEVER auto-committed, skipped while tree tasks are live, fail-soft per root. (v6.91) Two triggers, both detection-only on the supervisor event-drain thread with the git chain running on a bounded off-loop daemon thread (`supervisor/events._spawn_coop_checkpoint`; in-flight latch per root, quiescence re-validated before the git mutation, loud-fail receipt): root task_done with zero live members, AND tree quiescence — when the LAST live subtree member terminalizes under an already-settled root (closes the budget-death gap where the root always died before its children and the pile stayed uncommitted forever)
      ...
      ├── workspace_admission.py ← (v6.58.0) Workspace-task admission SSOT: the ONE workspace-root validator (git-worktree root, no repo/data overlap) shared by `/api/tasks` and the promote path; `resolve_room_workspace` defaults a project-room task to the room's registered `working_dir` (sentinel `workspace="none"` opts out) and LOUD-FAILS a set-but-broken working_dir (a room task must never silently degrade to a workspace-less self_modification-profile task); `compose_workspace_block` renders the shared [HEADLESS_WORKSPACE] guidance; `bounded_workspace_preflight` hard-caps the promote-path snapshot so the supervisor event-drain thread stays responsive. (Q10=A, 2026-08-08) A project promoted with an EMPTY working_dir AUTO-PROVISIONS one via `projects_registry.ensure_project_workspace` (idempotent genesis git repo under the durable subagent-projects root, validated by the same SSOT) and binds it — file-less project tasks stop running as workspace-less self_modification-profile tasks over the system repo. Binding side effects, disclosed: the task runs the EXTERNAL tool profile aimed at the project tree (self-repo reads need explicit `root="system_repo"`), memory forks to an isolated child drive, the one-writer project lease serializes on the folder, and the promote pays the bounded ≤8s preflight. Provision fires ONLY on the empty string — a non-empty broken working_dir still LOUD-FAILS (never blind-ensured over); `workspace="none"` still opts out; provisioning failure fails the promote loudly (`workspace_provisioning_failed`), never a silent file-less fallback. The doc matrix is deliberately NOT coupled to this binding (D-ARCH): ARCHITECTURE.md stays full-resident in max regardless of the workspace
      ...
      ├── reviewer_slot_config.py ← (6.1) Reviewer-slot configuration SSOT: ONE structured setting (`OUROBOROS_REVIEWER_SLOTS`, JSON `{triad[], scope[], advisory}`) with stable owner-assigned `slot_id`s (never array indexes), a per-row route `{kind, target_id}` (`api_chat` model id or opaque `harness[=model]` session spec — no `::` syntax, no vendor names in kinds) and a per-row effort on the existing `EFFORT_SCALE`. Absent → the legacy comma-lists + phase-5 route envs are read as the migration source (positional slot ids preserved so old receipts keep lining up; global Review/Scope efforts copied into rows; a legacy SESSION row migrates to the SHARED session harness as its route identity — never its comma-list model, which was never a harness — with effort in the field). The per-slot effort field is the ONE effort SSOT (D1/6.3): any effort embedded in a target identity is dropped so the field can never be silently overridden. When every commit-triad (or scope) row is delegated, the API-pinned surfaces fall back to the shipped default models and spend API budget — disclosed loudly (D4), never silent: a save-time warning, a durable `data/state/reviewer_slot_api_fallback.json` record, and honest UI text; the save is not blocked (recommendation A). Present → the legacy comma keys become a runtime projection of the api_chat rows for the API-pinned surfaces (plan review, task acceptance, skill review — D15), applied by `apply_settings_to_env`; there is no permanent double-write. Consumers: the commit triad reads `commit_triad_delivery()` (aligned per-row vectors), scope reads `scope_reviewer_slots()` (SSOT-aware), the advisory reads `advisory_slot_config()` (enable/kind/target/effort; disabling is a standing owner decision recorded as an AUDITED BYPASS by the pre-commit gate; on kind=api the target is the Claude-SDK model spelling — '' = `resolve_claude_code_model()` default — and the api route runs at the row's own effort, not the scope reviewer's). Malformed configuration raises ValueError row-precisely: refused with 400 at settings save, and turned into a typed loud refusal at review time on commit triad/scope/advisory (REVIEW_BLOCKED infra failure), plan review, and skill review (`reviewer_slot_config_error`, #116) — never a silent transport pick on those surfaces; task acceptance is the one owner-approved residual (2026-08 carve-out): on a malformed structured config it still runs on the projected legacy/default env panel. Also owns the D22 «выполняется как» projection: `record_reviewer_slot_executions` (called by the review coordinator after every run) writes each slot's last EFFECTIVE route/model/verdict-method, the applied credential `profile_id` and `access` the engine receipt disclosed, plus its capability_delta list to `data/state/reviewer_slot_last_execution.json` (NO applied effort: none exists upstream — no receipt or telemetry discloses a resolved effort — so the key is omitted rather than echoing the request as if it were the applied value; the REQUESTED effort is recorded beside it. NO containment either: that lives in the engine's own `attempts/<id>/attempt.yaml`. This file is one of FOUR separate applied stores and is not a join of them) (canonical data plane, atomic, ≤64 rows), and `reviewer_slot_last_executions` serves it back to the Models page — disclosure only, nothing routes off it.
      ...
      │   ├── task_contract.py ← Canonical per-task contract draft/resource normalization helpers, including advisory `acceptance_claims` (`claim`/`surface`/`support`/`priority`) for host-built support_refs; `effective_acceptance_claims(task, closed_plan_wave)` (W2) is the ONE pure seam resolving the claims that bind a task — ingress-contract claims first, the CLOSED plan wave's frozen `scope.acceptance_claims` only when ingress is empty — read by the acceptance-evidence packet and the child-contract builder; the running task contract is NEVER rebuilt mid-task (plan-frozen claims live in `plan_review_state` and are resolved at read time)
      ...
      │   ├── edit_ops.py       ← Advanced repo editing tools (editbench-validated): `apply_patch` (context-anchored V4A-style multi-file patch — hunks match by exact context lines plus optional `@@` anchors, trailing-whitespace fuzzy fallback, validated across all files/hunks before the first write, with per-hunk diagnostics) and `edit_batch` (batch of COUNTED exact replacements, validated before the first write: each edit declares the occurrence count it expects and replaces all of them; any mismatch aborts the whole batch). Repo lanes only (active_workspace/system_repo) via the same guard chain as edit_text; shared `_syntax_check`/`_unified_diff` helpers also back write_file's pre-write syntax guard and overwrite diff (an `edit_sketch` fast-apply tool was evaluated on editbench and removed — see devtools/benchmarks/editbench/README.md)
...
site/                         ← Public GitHub Pages source (Vite). `site/scripts/sync-assets.mjs` copies canonical `assets/` images into the gitignored `site/public/assets/`, and `pnpm build` renders into committed `docs/`. The visual homepage is joined by text-first `/about/`, `/install/`, and `/benchmarks/` routes, plus `/llms.txt`, `/install.json`, and the canonical sitemap. README.md's Benchmarks table remains the claim source of truth; the site, social images, and SVG status labels project the same scores and evidence state. `tests/test_public_site_metadata.py` guards canonical URLs, structured metadata, install data, asset hashes, and source-to-Pages sync.
```
...
Both resolvers search `platform_layer.bundled_resource_bases()` — the SSOT for
every bundled payload. Its first base is `OUROBOROS_BUNDLE_DIR`, exported by
`launcher.start_agent` and `packaged_cli._inner_cli_env`, because the process
that CONSUMES a bundled payload is normally not the frozen launcher: the server
and the CLI run as separate children of the embedded interpreter out of the
managed repo under the data dir, so they have no `sys._MEIPASS` and their
module parent is the repo, not the bundle. Without the handed-down root, every
bundled payload reads as absent in exactly the installation that ships it. The
remaining bases (`sys._MEIPASS`, repo root) serve the frozen process and dev
builds.

The packaged interpreter also lives inside the signed bundle, so both of
Python's default write targets are redirected in one place —
`launcher_bootstrap.embedded_python_env()` sets `PYTHONPYCACHEPREFIX` and
`PYTHONUSERBASE` under `<data>/state/`, and
`platform_layer.pip_install_target_args()` adds `--user` for exactly the
embedded interpreter (never for a dev venv, which refuses `--user`). All six pip
call sites route through it — including `supervisor/git_ops.sync_runtime_dependencies`,
the self-update dependency sync, which was the one left bare — and `install_deps`
returns pip's success instead of discarding its exit code.
...
configured depth/cap limits. Depth no longer rewrites the model lane: recursion is
bounded by the structural depth cap and by concurrency, and power comes from the
requested lane alone. Enabled/reviewed extension and
...
Plan review uses the same task-scoped artifact directory for two audit-only
projections written by `task_results.py`: `plan_task_handoffs.json` is the latest
mutable audit view, while `plan_task_handoffs.<fingerprint>.json` is the host's
write-once first-panel scout snapshot for retry continuity. Neither file grants
review authority or forms a security boundary against deliberate self-tampering by
the same root task; `plan_review_state` in the task result remains the SSOT, and
both files share the containing task artifact directory's retention/lifecycle.
...
**Authority** — what the child may DO — comes from `write_surface` through
`task_constraint` and `tool_access.active_tool_profile`. **Power** — how strong the
child is — comes from `model_lane` (`auto`, `main`, `heavy`, `light`) and nothing
else. **Executor** — who runs the child — comes from `executor`, a closed enum of
intents (`auto`, `harness`, `native`) rather than a harness name, so adding a harness
never touches this contract. Until v6.87.7 `auto` resolved to Heavy whenever the child
was "mutating", where mutating meant `write_surface OR may_mutate` — and `may_mutate`
...
removed in the same change — they named a fan-out TOPOLOGY smuggled through a strength
parameter, and no review surface ever used them (commit, plan, acceptance and skill
review all read their slots from config and run on the review substrate). Durable
...
and TOOL profile are DERIVED. The CREDENTIAL profile is not: nothing host-side
derives it (`subagents.py`: "WHICH profile a run lands on is Claudexor's business"),
it is either a manual per-row pin or the daemon's own rotation, and the APPLIED one
is only ever read back from the engine receipt into a different store. `schedule_subagent` published an `effort`
...
receipts — status plus the shared `receipt_identity_projection` identity, hard cap 10
with the exact omitted count via `disclosed_list_projection` — so
a parent absorbs a child on receipt-level green/red instead of prose. The rows
resolve against the canonical status root, which a forked/empty child never writes:
child finalization publishes the child-drive `verification_receipts.jsonl` beside
the artifact rebase (`headless.copy_child_task_result` →
`_publish_child_verification_receipts`; receipts are keyed by `(drive_root,
task_id)` and the canonical file for a child task_id has no other writer, so the
whole-file copy is a collision-free idempotent refresh that survives child-drive
pruning), and until a canonical copy exists the reader falls back to the child
drive recorded on the effective result, so the rows are never silently absent in
the pre-copy-back window the parent most often absorbs the child in. The bound is
OUTSTANDING-first, then newest: every still-unreconciled red / masked pass (the
shared `_outcome_receipts.unreconciled_failed`/`unreconciled_masked` SSOT) is carried
and tagged `outstanding`, then the newest remaining rows fill the cap. A plain
newest-10 window let a child that failed a check early and then produced ten greens
hand the parent an affirmatively all-green list with the red only implied by a count,
which is the one thing the absorption decision must not be able to miss; the
...
here — the parent contract was located with a type test that let `ToolContext`'s empty
...
the child task result — its artifact files rebased into the canonical store and
its durable `verify_and_record` receipts published to the canonical receipts
path (the W2 receipt-level handoff reads receipts parent-side) — is copied back
to the parent drive; identity, scratchpad,
...
│   │   │   └── archived/ ← Retired settled continuations. Writer: `task_continuation.retire_settled_continuations` (worker, at Review Continuity context build in `agent_task_pipeline`) MOVES a continuation here — collision-safe timestamped rename — once its owning task reached a SETTLED status and the record sat un-resumed ≥7 days (`RETIRE_SETTLED_CONTINUATION_AFTER_DAYS`; a fresh settled record deliberately stays live as the cross-task resume pointer), so stale records stop riding into every new task's Review Continuity section. Reader: none at runtime (`list_review_continuations` scans only the live dir) — forensic/manual recovery only. Lifecycle: durable, never deleted (P1); any retirement error leaves the file live (fail-open: context noise over lost review state); malformed files go to the `corrupt/` quarantine, never here
...
own label and its parent's (via `connections.ratchet_scope_sensitivity`, which
keeps the rule next to the sensitivity vocabulary), so a subtask cannot widen where
its parent's data may travel. Enforcing it at that one choke point rather than at
each call site is deliberate: too many places build a child scope for "remember to propagate it" to
...
place every physical send lands; it hands the outcome to
`connection_pool.note_attempt_outcome`, which owns the classification (accounting
owns money, routing owns what counts as a credential failure). A failure cools the
connection down (a bad credential rests far longer than a transient error, since it
will keep failing), a success clears it. Cooldowns are process-local and time-based on purpose — a hint
...
state/campaign writes that resolve to the captured live data root.

The hermetic commit gate mirrors CI's split instead of running one serial pass: a
...
Chat instances carry a real lifecycle (P3): every resource an instance acquires has a paired release — `ws.on()` returns a disposer (listener sets iterate over a snapshot, so subscriptions during emit do not receive that emit), and `destroy()` disposes all ws subscriptions, removes the instance's window/document listeners, disconnects its ResizeObserver, clears every timer, drops the buffered collections, and removes the DOM subtree last; late rAF loops and async continuations (history sync, preference loads, full-line fetches, the paint receipt) no-op on the `destroyed` flag so a torn-down instance can never repaint or acknowledge a revision. `app.js` enforces a single-live-panel policy: opening or closing a Project destroys any other project instance instead of hiding it — the one exception is an instance holding unsendable client state (staged attachments or an in-flight upload), which is hidden and marked `data-pending-work` so switching pages mid-upload never drops attachments. The pending-work survivor is not a permanent exception: reopening its project reuses the instance and clears the marker, and once its uploads finish it is destroyed like any other panel the next time it is closed or switched away. Scroll intent survives destruction in a per-project stash re-applied after the recreated instance's first paint, the typed-but-unsent input draft persists per thread in sessionStorage, and reopening an already-painted instance skips the forced history refetch (`hasPaintedHistory` gates `forcePaint`).
...
History/log reads are bounded, projection-over-replay (v6.90.x P2). `logs/progress.jsonl` now rotates like chat on the same supervisor tick (`supervisor/state.rotate_jsonl_log_if_needed`, the generalized rotator behind the `rotate_chat_log_if_needed` wrapper; suppressed in `.ouroboros_isolated_benchmark` roots, whose harness readers — e.g. the TB ATIF builder, itself archive-chain-aware — expect trial-local single files; log archives are durable history and belong to NO GC sweep). All hot readers are archive-aware and bounded through one primitive, `gateway/_helpers.read_rotated_jsonl_entries` (built on `iter_jsonl_objects(tail_bytes=…)`): a 512KB live byte tail that doubles until the FILTERED per-type quota is satisfied (degenerate case = one full read) plus the newest-first archive backfill — used by `/api/chat/history` for both chat and progress and by `/api/logs/{name}`. `_annotate_terminal_task_truth` runs AFTER quota slicing on exactly the emitted window (terminal truth lands on the latest in-window row), the task→project-chat bindings map is preloaded once per request, and every display-path effective-result read passes `materialize_artifacts=False` — a status/cost projection that skips the artifact block (including the mutating child-artifact rebase) and carries no disposition/sha claims, while sha-economy consumers (join_ledger, wait/get result, api_task_get/artifact, reconcile, prune) keep the materializing default. The SSE stream `/api/tasks/{id}/events` replays with one full archive-aware merge, then follows via per-log byte offsets: each tick reads only appended bytes, re-discovers late-spawned child roots (their logs join at offset 0), heals mid-stream rotation from the newest archive's unconsumed suffix, recomputes the terminal projection only when offsets advanced or the queue snapshot moved, and emits incrementally only while new rows sort strictly after the emitted tail AND the lineage filter did not grow — otherwise one full re-merge resumes from the client cursor. The terminal `task_result` event is the one sanctioned exception to the projection rule: it is emitted from a single materializing read at termination, so watching a task to completion still delivers the artifact-bearing payload and runs the read-repair rebase once per stream (`/api/tasks/{id}` remains the authoritative terminal read). seq is monotonic in-stream; across reconnects the cursor contract is positional and delivery is at-least-once (pre-existing property, disclosed) — and a retro-inserted row that sorts before the client's cursor is dropped for that stream rather than duplicated (pre-existing parity, narrowed by the follower's re-merge triggers; a from-zero replay recovers it).
...
Settings has Providers, Secrets, Models, Behavior, Advanced, and About. It handles provider keys, model routing, review settings, runtime mode, the official update channel, external skills repo, ClawHub registry URL, MCP servers, source control metadata, local model runtime, extension settings, timeouts, and reset. Rarely used provider cards (Cloud.ru, MiniMax, GigaChat) live under a collapsed "More providers" section that auto-opens when a usable provider credential is configured (a Cloud.ru key, a MiniMax key, a GigaChat OAuth credential, or a complete GigaChat basic-auth pair — base-URL/scope/TLS/region fields always carry shipped defaults and never count); the collapsed inputs stay mounted in the DOM so settings load/save applies to them unchanged. Hot-reload policy: total budget, timeouts, GitHub metadata, and update channel apply immediately; per-task cost threshold, models, API keys, effort, and review settings apply next task; local runtime, worker count, base URLs, provider runtime parameters, and runtime-mode changes require restart. Context mode hot-applies through the dedicated owner endpoint without a restart; lowering Max to Low is refused while queued, running, or direct-chat work exists. Runtime mode remains owner-controlled: ordinary `/api/settings` drops it, while `/api/owner/runtime-mode` persists the next-boot value without changing the current boot baseline.
...
- `POST /api/settings` (v6.80.0) additionally PROBES the configured review and scope-review slots' own routes (never the main lane's `USE_LOCAL_MAIN` routing) and returns any scope slot that does not confirm ≥1M as `review_capability_notices[]` using the SAME `needs_ack:{route, route_fp, evidence}` payload shape the Max-context gate already returns, so the existing owner-ack UI flow applies unchanged. Before this, the Max gate probed only the MAIN route, so a PINNED scope reviewer had no path to `confirmed` evidence at all and silently ran in the conservative sub-floor window; `scope_review._scope_window` additionally performs a metadata-only (never generative, never billed) probe for EVERY scope route, rate-limited by the evidence TTL rather than by the process lifetime (v6.87.45: a per-process memo outlived the 24h record, so an install that stayed up past the TTL read its own reviewer as EXPIRED forever and blocked every commit), the shipped default included (v6.87.44: exempting the default left the one route that gates commits unable to source its window, so its 1M was permanently invented). Both surfaces are ROUTE-aware, not model-aware: the probe is keyed by the full `capability_evidence.route_fingerprint` and the save-time notice fires on any route-affecting change (`_REVIEW_ROUTE_BASE_URL_KEYS` — `OPENAI_BASE_URL`, `OPENAI_COMPATIBLE_BASE_URL`, `CLOUDRU_FOUNDATION_MODELS_BASE_URL`, `GIGACHAT_BASE_URL`, `MINIMAX_REGION`) as well as a slot change, and the slot itself is read from the CANDIDATE settings rather than process env. Keyed by model name alone, a hot base-URL change produced an unprobed route with no notice: the next scope review fell silently to the conservative sub-floor and the advertised owner-ack path was unreachable. Save also warns loudly about review model ids the provider catalog does not list (a truncated slot value such as `-5` used to surface only as three waves of `400 ... is not a valid model ID`, destroying the review quorum); nothing is rejected or rewritten, and fail-closed-on-absent-evidence is unchanged — a pin is routing intent, never an owner-ack.
...
- Managed update boundary: `GET /api/update/status` is passive and may overlay only a cache bound to the same channel/remote/ref; `POST /api/update/preflight` performs the bounded official fetch and a real isolated 3-way plan, returning exact `base_sha` + `target_sha`. A clean fast-forward lands the official target itself, so an untouched local `ouroboros` remains identical to the promoted branch; a clean divergence creates one merge commit whose parents are the reviewed HEAD and the official target. Dirty/untracked work never enters committed history on a clean auto-update: planning uses a synthetic local snapshot only to classify conflicts against local edits, while the apply stashes the dirty work (tracked + untracked, recorded as `stash_sha` on the durable tx) and restores it as uncommitted content — after boot finalization on success, or onto the exact pre-update tree on rollback. A restore that would conflict with the updated tree keeps the stash entry and discloses the exact `git stash apply` recovery command instead of guessing. Apply requires the disclosed SHAs, takes `data/locks/managed_update.lock`, closes new repository writers, drains direct/ephemeral turns, proves the worker pool stopped, stops tracked services, and rebuilds the same plan before mutation; the complete fenced transaction runs off the gateway event loop so Panic remains responsive. Every clean plan lands deterministically through the supervisor, including a clean merge over a dirty worktree (the dirty work rides the stash, not the commit). Only a real Git conflict, regardless of filename, goes to the one tx-authorized assisted resolver and the normal triad/scope `commit_reviewed` path. Before live conflict materialization, one fresh resolver must finish booting from the exact base SHA, so ordinary conflicts in importable runtime files remain editable. A host crash while startup-critical conflict markers are still unresolved may require rescue-ref recovery; moving assisted resolution into an isolated worktree is deliberately deferred. Resolver authority requires both the durable tx and its matching host-enqueued task-metadata fingerprint, so an agent-written marker cannot mint write or rollback authority. That resolver alone may edit ordinary runtime-protected paths for this transaction; global runtime-mode policy is unchanged for every other task. `replace` is not selected by ordinary Update: it is an explicitly confirmed recovery action that resets to the same pinned official SHA after rescue. Both paths write the durable tx before mutation, persist pre-restart smoke as `pending` then `passed` (boot re-runs a still-pending proof), verify rollback before clearing intent/tx markers, and retain `gate_blocked` when rollback or cleanup cannot be proven. Managed merge tests pass before restart and roll a red update back; the ordinary self-modification commit/tag/test/push contract is unchanged. `server.py` finalizes or recovers transactions that leave the startup tree importable, then performs one target-bound update check.
...
1. `rotate_chat_log_if_needed()` / `rotate_jsonl_log_if_needed()` — archive chat.jsonl and progress.jsonl if > 800KB (suppressed in isolated benchmark roots)
...
Host-enforced task acceptance is root-owned and structurally gated, not turn-counted or keyword-gated. `off` disables it. In `auto` and `required`, queued/headless/scheduled substantive roots are reviewed. Direct chat is reviewed after an observable reviewable effect or a typed deliverable/criterion; ordinary read-only tool activity, pure conversation, and meta/routing-control turns are skipped. Reviewable effects are successful commits, non-scratch writes/edits, declared process outputs, integrated child patches, or a registered canonical artifact; cognitive-memory updates are not effects. Child tool reviews remain advisory evidence and are superseded by the root verdict rather than becoming a second authority.
...
Context compaction is failure-isolated (v6.28.0). `context_compaction.py` summarizes old rounds in batches with per-batch isolation: a failed batch leaves only its own rounds raw and its spend is still accounted (`_BatchSummaryError` carries usage); a round whose summary is missing degrades individually instead of failing the batch. The summarizer prefers a structured `emit_round_summaries` tool protocol (`tool_choice="required"`, reliable `round_id` keying) and falls back to the legacy `[round:N]` text protocol — automatically for local light models, or per-response when a model answers in prose. ⚠️-protection scans the first two non-empty lines of a tool result (shell autocorrect notes — with or without a blank separator line — can prefix the marker), and `⚠️ SHELL_EXIT_ERROR` rounds are deliberately compactable — failed-command trial-and-error history is exactly what must compact, with the summarizer instructed to keep the first error line verbatim. Emergency compaction in `loop.py` adapts `keep_recent` to `min(50, max(6, spans//2), max(1, spans-1))` — halve the history with floor 6, always clamped below the span count — so an oversized transcript with few huge rounds actually compacts instead of no-opping at the `len(spans) <= keep_recent` gate (a single round has nothing older to summarize). Its trigger splits NECESSITY from UTILITY. NECESSITY ("compact at all?") is TOTAL calibrated pressure: the whole round payload — transcript, the system block, AND the tool schemas that travel beside `messages` on the wire (counted through the one `context_fit.tool_schema_tokens` seam; a transcript-only measure fired a whole ~148K-char tool envelope late) — compared against the mode's char budget in real tokens via `context_fit.main_loop_token_density` (neutral 1.0 cold, MEASURED supersedes; never the review-pack cold-conservative density, which would demote fresh installs). UTILITY ("can a pass help, when should it refire?") is the COMPACTABLE region only — the transcript beyond the frozen frame. Two shapes arm the hysteresis in accumulated usage, both through one seam with one loud `emit_progress` disclosure plus a typed `compaction_hysteresis_armed` checkpoint event (carrying the calibrated tokens, region size, schema tokens and density): a pass that RAN and could not bring total pressure back under the trigger (`emergency_pass_futile`), and a transcript holding under two tool rounds, where the compactor's `len(spans) <= keep_recent` gate makes the pass a structural no-op (`nothing_compactable`) — a low-mode task can enter its very first rounds with the frame alone over the trigger. No further pass fires until the region grows ≥`COMPACTION_HYSTERESIS_REGION_GROWTH` (floored at `armed + 1`, so an EMPTY armed region still suppresses — a 20%-growth test on zero never would) or `COMPACTION_HYSTERESIS_ROUNDS` pass. This is what stopped the submarine thrash where an irreducible frame above the LOW threshold re-fired the summarizer and rewrote the transcript — destroying the prompt cache — on 35 of 35 rounds. The reactive provider-overflow one-shot Low retry is untouched.
...
same-id subagent retry. A grace window that is no longer wanted is RETRACTED,
not merely forgotten (v6.87.34): the control's msg_id is the episode's identity
and is stored beside the latch, `task_reaper.withdraw_finalization_grace` (the
symmetric twin of `request_finalization_grace`) revokes the durable control and
clears the latch together — refusing to clear either half alone — and the
mailbox resolves `control_revoked` entries over the whole file before delivery,
so a control revoked before its reader drained it is never seen. Withdrawal is
keyed on the TASK's own progress: a progressing descendant spares an
orchestrator from the kill but is not the orchestrator answering, so it cannot
re-arm the episode on every subtree flicker. What counts as the task's own
progress is settled at the two stamping sites of `last_progress_at`, both in
`supervisor/events.py`: `_handle_send_message` for progress narration frames
and `_handle_llm_usage` for completed LLM rounds (both genuinely
task-authored). A progress frame's `task_id` is a
ROUTING address, not a claim of authorship, so a frame the SUPERVISOR authored
about a task — every terminal-path toast — declares `HOST_NARRATION` and is
never stamped as the task's work (v6.87.35; before it, the grace toast stamped
the task it announced, the 0.5s tick read that as the task resuming, and no
idle-timeout episode on the real wire ever survived to reach its kill). The
episode's clock is likewise suspended while the task is SPARED: sparing means
the supervisor is not stopping it this tick, so the window measures intended-stop
time, not wall clock — otherwise a parent blocked in `wait_tasks` (no own
progress, no mailbox drains between rounds) was killed at the first tick its
subtree went quiet, with the `finalize_now` still unread. One episode, one
control, one toast, and a whole window whenever sparing ends
(`task_reaper.resolve_grace_episode_for_spared_task`).
...
Every tool call passes hardcoded registry sandbox first, then policy-based LLM safety when required. The LLM layer's coverage is owner-selected via `OUROBOROS_SAFETY_MODE` (see the `/api/owner/safety-mode` contract above); the deterministic layers run in every mode. `runtime_mode_policy.py` defines protected paths: safety-critical files, frozen contracts, release/build/managed-repo invariants. Light mode blocks Ouroboros self-repo/control-plane mutation, not ordinary user-file creation: `write_file(root=user_files|task_drive|artifact_store)`, and process cwd under those roots remain valid. Task-local git (including commits) is allowed in every shell lane — external workspaces AND the default non-workspace lane (direct chat, light, self_modification-profile tasks) — through one target-aware policy (`git_shell_policy.external_workspace_git_violation`): only MUTATING git that targets the Ouroboros runtime (system repo / any data drive; bidirectional, casefold, symlink-resolved containment) is blocked with the `commit_reviewed` remedy, read-only git works everywhere including at the runtime, `allowed_resources.network=false` still fences network subcommands, and acting `self_worktree` children keep the strict read-only git policy (a moved HEAD fails patch capture). Two composition rules make "read-only everywhere / mutating free outside the runtime" actually hold rather than merely be stated. (1) `git init <dir>` and `git clone <url> <dir>` are judged by their DESTINATION, not by the cwd — the default lane's default cwd IS the system repo, so a cwd-based verdict refused `git init ~/projects/foo` from direct chat and light mode; the destination is parsed structurally (the last path-shaped operand, with init/clone's own value-taking flags such as `-b`/`--depth` consumed, so `git init -b main /tmp/proj` and `git clone --depth 1 <url> /tmp/proj` resolve the real destination); with no explicit destination the cwd is the target and the containment check applies unchanged, and a `--flag=<path>` retarget such as `--separate-git-dir` is still checked — by the flag's documented type, so even a bare-name value is resolved, while values of non-path flags (`-b feature/x`, `--revision refs/heads/x`) are never mistaken for paths. Relative destinations and arguments are canonicalized against the effective base like absolute ones, so a relative destination resolving back INTO the runtime is refused. (2) In external-workspace mode the runtime/secret READ guard (which stops raw `cat`/`python -c open(...)` from reaching the runtime or credentials) EXEMPTS commands that `git_shell_policy.is_readonly_git_command` proves are read-only git in every segment; the exemption is all-or-nothing per segment (`git status && cat <data>/settings.json` is not exempt) AND write-aware: a read-only subcommand carrying the diff `--output=<file>` option is NOT read-only (real git truncates `<file>`; the write is judged at the file, exactly like init/clone's destination, so `git log --output=<runtime path>` is refused while `git log --output=/tmp/x` stays free), and `--no-index` — which turns `git diff`/`git grep` into an arbitrary-file reader — disqualifies the exemption too, so the credential surface genuinely stays closed. The shell cwd is resolved exactly once per safety check, through `resolve_shell_cwd`, and the canonical path is passed to each guard (never a raw cwd label re-joined onto a root — the v6.74.0 D1 class). In external workspace mode patch artifacts are captured against the preflight git base. Light still blocks `runtime_data` as an artifact workaround, direct repo writes, native/control-plane skill paths, state/memory/settings, VCS mutation against the Ouroboros repo, and runtime-mode self-elevation. Advanced can evolve normal app code; pro can leave protected edits on disk but the commit still requires review.
...
(v6.87.9) Both paths pin the delegated run's AMBIENT trust surface, which the
SDK otherwise inherits from the directory being worked on: `setting_sources=[]`
and `strict_mcp_config=True` keep a foreign `.claude/settings.json` (hooks, env,
permission rules) and `.mcp.json` out of the run — `--print` skips the
workspace-trust prompt that would otherwise gate them. The tool surface is
CLOSED, not merely permission-listed: `READONLY_TOOLS`/`EDIT_TOOLS` are the
single grant, and the base `tools` set, the CLI deny list, and a PreToolUse
allowlist hook are all derived from it, so subagent tools (`Agent`/`Task`) and
anything a later CLI release or a foreign MCP server adds arrive DENIED by
absence. Reads are confined to the working directory by a read guard symmetric
with the write guard (both sides resolved, so symlink escapes are caught) —
`allowed_tools` alone pre-approves `Read`/`Grep`/`Glob` for the whole
filesystem. Accounting survives a kill: the advisory child publishes its attempt
identity and running token totals to the parent on stdout, and a timeout, abort,
or unparseable result makes the parent terminalize that reservation
(`usage_accounting.terminalize_abandoned_attempt`) — settled from the measured
usage, or honestly `unresolved`. Before this, the advisory reservation (which is
the whole REMAINING budget) stayed non-terminal forever and every later model
call was refused as budget-exhausted.

### Usage ledger substrate vs. accounting policy

`ouroboros/usage_ledger.py` owns the durable append-only JSONL ledger itself:
cross-process locking, atomic append + fsync, structural validation of every row and
state transition, and quarantine of a torn tail. It knows what a well-formed ledger row
IS and nothing else. `ouroboros/usage_accounting.py` is the POLICY on top — scopes,
reservations, settlement, budget fences, pricing, projections, admission — and imports
FROM the substrate; the substrate never imports back.

Rationale: the file is the monetary authority (P8 budget integrity), so a change in
accounting policy must not be able to corrupt it, and a locking or fsync fix must not
be able to silently alter what a reservation means. The one-way seam makes both
directions structurally impossible rather than merely discouraged. The split was forced
by a concrete failure: three concurrent branches each added a modest amount and the
merged module crossed the 1600-line health gate that no single branch broke — the gate
correctly reported that the module had two jobs. Private substrate names are re-bound in
`usage_accounting` so existing call sites (and tests that monkeypatch `_locked`) are
unaffected.
...
**Absence is an answer, not ignorance.** `daemon_says_absent` (HTTP 404) is the one
predicate for "the daemon told us this is not there", and it is read at every custody
surface. A run the daemon does not have is not mutating anything, so it is CLOSED
(`delegate_run_closed_absent`) rather than settled — there is no terminal detail, and
inventing a `$0.00` ledger row for a run nobody can describe is the one shape the ledger
must never produce. A registration the daemon does not have is already retired, so the
obligation is discharged. Reading either as "unreachable" left the run in `open_runs`,
re-faulted on every sweep, with a CRITICAL nothing could ever clear. A daemon that is
merely unreachable still faults: absence and ignorance stay apart.
...
**Execution rule table** (`subagents.resolve_subagent_executor`, a pure function whose
health facts are inputs): `auto` + no route → native; `auto` + healthy → nanny; `auto` +
spent window → nanny plus a typed `reset_at` so it can decide (wait / partial / native);
`auto` + otherwise unavailable → native with a visible marker; explicit `harness` +
unavailable → a typed BLOCKER, because silently spending API money is the one outcome
an explicit harness request must never produce; `native` → always native.
...
**The model cannot widen its own authority.** `delegate_start` exposes exactly two
parameters, `prompt` and `max_seconds`; there is no access, mode, isolation, root or
scope argument to set, so the child can ask the host to run something but never choose
with what powers. Every delegated run also carries host-authored `instructions` stating
...
poison credential planning, pricing, and benchmark provenance.

### Git and commit review

`tools/git.py` owns repo writes, staging, commit, rollback/revert/restore, auto-tag, auto-push, and CI-status follow-up. `write_file` writes without committing (overwrites of existing repo files append the unified diff vs the previous version, and syntactically invalid `.py`/`.json` content is blocked before writing — `force=true` bypasses with an explicit `SYNTAX_GUARD_BYPASSED` disclosure); `edit_text` does exact one-occurrence edits; `edit_batch` applies an atomic batch of counted exact replacements; `apply_patch` applies an atomic context-anchored multi-file patch; `commit_reviewed` stages, checks advisory freshness, runs deterministic preflight, runs triad + scope review, revalidates the exact Git binding, commits, verifies, tags, and pushes. The binding is the `git write-tree` SHA, ordered `HEAD`/`MERGE_HEAD` parents, indexed VERSION, expected `v{VERSION}` tag, any existing tag target, and binary staged-diff hash. A changed parent/tag is therefore as invalidating as a changed file; an existing release tag is never silently accepted or retargeted. After `git commit`, tree/parents/VERSION and tag target are re-read before a success attempt or push is recorded.
...
Reference-doc forms are unified per the D-ARCH owner decision (2026-08-08): ARCHITECTURE.md follows the OWNER CONTEXT MODE alone — full-resident in max for EVERY task class (self-body, project tasks with or without a folder, evolution, external/headless/delegated surfaces), the lossless navigation map in low. Rationale (owner, recorded so it is not lost): architecture.md is Ouroboros's capability/tools/access map; it stays resident in max even for project/evolution work because without it the agent cannot reason about HOW to work effectively — context economy comes from dropping DEVELOPMENT.md for project work, never ARCHITECTURE. DEVELOPMENT.md (the self-engineering handbook) adapts MODE-INDEPENDENTLY, per the D-DEV owner decision (2026-08-08): it loads exactly when the work targets Ouroboros's own body, and the structural signal is the ACTIVE REPO BINDING — a path fact, never a guess from message text (P5). Concretely `not _task_uses_external_context(task)`: a bound workspace (including a project task's auto-provisioned genesis tree), a subagent, or an api/cli/scheduled surface means the work is on ANOTHER codebase and gets the visible on-demand pointer; everything still bound to the system repo keeps the handbook full. Project MEMBERSHIP is deliberately NOT the signal — a direct-chat turn in a project ROOM binds no workspace and therefore KEEPS DEVELOPMENT.md, whereas a project task with a bound folder does not. `workspace="none"` binds nothing, so such a task keeps it (its own territory); evolution/self-body keep it through the self-body branch; explicit `context_requires_development` / `context_requires_self_body_docs` always win (both are task-dict-level overrides; only `context_requires_self_body_docs` is a `POST /api/tasks` body field — `context_requires_development` is an internal lever for structured in-process callers, deliberately not public API surface). The doc decision is deliberately DECOUPLED from workspace binding — binding a workspace at promotion fixes paths, tool profile and the lease, but never drags the capability map out of a max context (the former per-task `force_low_docs` lever is gone).
...
- **Canonical chat and owner routing**: Web, CLI, and existing owner transports enter the same router. In Main, the ordinary decision turn gets a compact manifest of Projects, RUNNING/PENDING roots, recent final results/artifacts, and recent canonical dialogue; if there are no Projects/tasks it retains the prior direct flow. In a Project room, exactly one addressable RUNNING/PENDING root is normally a zero-call mailbox delivery. Explicit Swarm is the narrow exception: it always enters the ephemeral LLM router, even in an empty Main or a one-root Project, and must admit one new managed root instead of steering or answering the work inline. Main may choose either a fresh Main root or an existing Project; a Project room is pinned to its host-owned current Project. The short router never creates a new Project or accepts model-supplied workspace/source scope during admission; the managed root may establish later scope through the ordinary task affordances. The first real admission attempt is latched in the short turn, so an unconfirmed/rejected receipt cannot emit a second root. `force_plan` transfers through that admission into the managed task, where `plan_task`, implementation, and conditional fan-out run. The normal acknowledgement remains LLM-authored from the exact routing receipt; only a genuine forced-exit rail uses deterministic host fallback text. Ordinary zero/multiple-candidate decisions retain the existing `needs_manual_target` behavior. `handle_chat_ephemeral`, `steer_task`, promote/bind/history/task-result lookup, attachment staging, and the existing serialized direct lane are reused—there is no parallel message lane. Each inbound row is stored once; Project history projects the binding-held source refs (immutable once valid; a ref-less binding may be one-way enriched), and when the canonical row has left the bounded read window the lens synthesizes the start message from the binding's own `source_text` (post-quota, identity-deduped, hard-capped — `origin_projected=true`); `chat_annotations.jsonl` carries only the latest owner-visible action/target/status. The typed annotation is presentation metadata attached to the owner's message, while the Agent normalizes blank model output to a visible warning and every finalized response remains a separate durable assistant reply across Web and non-Web transports. Inline Main replies create no routing annotation.
...
- **Sidebar thread-activity derivation**: `projects_summary` derives `has_thread_activity` from registry facts only — `origin=="owner_ui"`, any task binding for the project, `visible_revision > 0`, or the durable write-once `thread_activity_seen` flag; the former per-request chat/progress log scan is retired, so the GET path never scans logs and never writes. Legacy projects whose thread activity predates the `visible_revision` counter get the flag from a one-time archive-aware backfill at boot reconcile (`reconcile_projects` → `_backfill_thread_activity`, persisted through the registry's own `update_project` write path). Disclosed micro-delta: a new project whose thread carries only telemetry rows (no owner-visible canonical row, no binding) reads inactive until its first visible row — the junk-row shape the filter exists to hide.
...
"Where did the money go" for a ROOT task is a READ-SIDE projection, never a persisted monetary object (v6.91): `GET /api/tasks/{id}` on a root computes `cost_breakdown` = `{own_usd, children_usd (subtree − own), delegated_disclosed_usd, subscription_sessions, unknown_unmetered, non_final_rows, cost_final, authority}` from `usage_breakdown` at read time (`gateway/tasks.py::_task_cost_breakdown_view`); the ledger stays the single authority and the stored envelope keeps only its existing own/subtree fields. `usage_breakdown` itself gains a `delegated` bucket — a FILTER over the same rows by execution kind (`subscription_session`), never a third sum: disclosed-free sessions settle at $0 there, undisclosed spend stays unknown/unmetered. Non-root details omit the view (subtree math is ledger-attributable only at the root); an unreadable ledger yields an ABSENT field, never a confident `$0`.
...
| OUROBOROS_ALLOW_MUTATIVE_SUBAGENTS | (empty) | Allow mutative (acting) subagents. Empty = follow runtime mode, SURFACE-AWARE: every surface ON in advanced/pro; light allows `external_workspace`/`genesis` (they build outside the Ouroboros runtime) and keeps `self_worktree` OFF. Explicit true/false overrides for every surface. Owner-controlled. Settings exposes Off / Auto / On in Models → Subagents beside the delegation route (SC-4): Auto is the unset, mode-derived state — it saves the empty value so the surface-aware default keeps deciding — and unset displays as the truthful effective "On" in advanced/pro while displaying as "Auto" in light, where neither binary label is honest. (The earlier explicit-On/Off-only contract was valid only while the unset default was binary per mode; the surface-aware light default obsoleted it.) |
...
| OUROBOROS_PROMPT_CACHE_TTL | 1h | Global prompt-cache TTL (`default` \| `5m` \| `1h`), an HONEST override (owner decision 2026-08-08): consumed only by the send-time cache finalizer `llm._normalize_payload_cache_ttl`, which stamps the value onto EVERY existing `cache_control` breakpoint of the Anthropic-normalizing family (direct Anthropic + OpenRouter `anthropic/*`) — main loop, review lanes (`cached_prompt_blocks` projects this setting; the former `REVIEW_CACHE_TTL` constant is collapsed into it) and the safety supervisor alike — before the existing promotion rule, so TTL ordering stays legal by construction. `default` sends bare markers (provider default 5m tier) and leaves caller-declared TTLs standing. Non-Anthropic wire formats are untouched (Gemini has no documented ttl field; OpenAI/Grok/DeepSeek cache implicitly). The APPLIED TTL is reported in usage (`prompt_cache_ttl`) so pricing bills 1h writes at the documented 2×-vs-1.25× ratio (with Anthropic's per-tier `cache_creation` split honored when reported). |
...
- `ouroboros` is always the local working branch. Its name does not select the update source.
- The runtime setting selects one official feed: Stable → the newest plain `vX.Y.Z` release commit reachable from both `main` and `ouroboros-stable` (default), QA → the `ouroboros-stable` tip, Development → the `ouroboros` tip.
- Promotion is an owner-controlled exact-SHA operation: active development moves through `ouroboros`, QA validates the candidate on `ouroboros-stable`, then the same SHA moves to `main`. The branches may differ during development and converge after a stable release.
- The existing local `ouroboros-stable` ref also remains the generic import-test fallback maintained by `promote_to_stable`; that local recovery role neither selects the runtime feed nor makes the official remote QA branch recovery-only. Managed-update rollback restores its exact pre-update SHA instead.
- Desktop and Colab both reject an official target before checkout/apply unless its tree contains a non-empty regular `BIBLE.md`; before runtime helpers are importable, the notebook mirrors both that Git-tree predicate and the shared Stable-tag selector, so it never imports an untagged `main` tip first.
- On first Colab bootstrap the local fallback is seeded from the already-fetched shared Stable release even when QA/Development was selected; if no valid shared release is available, the already-validated selected channel is used instead. The fallback then remains pinned until an explicit promotion.
- Launcher metadata describes bootstrap provenance/local sync only. Runtime update status, preflight, Colab bootstrap, and apply use the selected channel and pin its exact fetched SHA; an old frozen launcher may keep its old metadata without changing that feed.
- Branch protection and the reviewed/tested candidate SHA are the release-eligibility boundary. Stable admission additionally requires the shared plain release tag above; QA and Development do not use tags or version comparisons as gates.
...
5. Build+release on `v*` tags: PyInstaller artifacts for macOS/Linux/Windows,
   smoke receipts from the final archives, digest-pinned Syft CycloneDX SBOMs,
   source-bound GitHub provenance and SBOM attestations, an exact checksum and
   evidence capsule, then a draft-to-published GitHub Release. The release job
   rechecks the remote annotated tag against the event SHA before creating the
   draft and again before publication. Apple credentials are step-scoped and
   are absent from the SBOM and attestation steps.
...
5. get_owned_daemon().stop()         ← kill the SELF-STARTED owned Claudexor
   │                                    daemon's whole process group, taking its
   │                                    delegated harness runs (children) with it.
   │                                    No network run-cancel calls — panic is
   │                                    instant. Disclosed residual: an ATTACHED
   │                                    daemon (one this process did not spawn)
   │                                    stays alive; the process-custody reaper
   │                                    collects it at the next manual start.
...
| `TaskDetailResponse` + `TaskCostBreakdown` (v6.91, additive-optional) — the `GET /api/tasks/{task_id}` detail envelope typed as an open shape (stored task-result keys pass through) plus the additive-optional `cost_breakdown` "where did the money go" projection: computed at READ TIME from the physical-attempt ledger by `gateway/tasks.py::_task_cost_breakdown_view`, never persisted, and emitted for ROOT task details only. When the object is present every key is present (`own_usd`, `children_usd`, `unattributed_usd`, `delegated_disclosed_usd`, `subscription_sessions`, `unknown_unmetered`, `non_final_rows`, `cost_final`, `authority: "physical_attempt_ledger"`; `own + children + unattributed == subtree`); the WHOLE object is absent — never a confident $0 — when accounting is unavailable or holds no attributable row for the subtree, and on non-root task details. | `ouroboros/gateway/contracts.py`, `ouroboros/gateway/tasks.py`, `web/modules/api_types.js` | `tests/test_gateway_parity.py` pins both types in the name-level and field-level JS-mirror parity lists, pins the emitted root view's key set to exactly `TaskCostBreakdown`'s declared keys (non-root → no object), and pins `TaskDetailResponse` as fully optional with `cost_breakdown` typed as `TaskCostBreakdown`. |
